...
namespace Dic\UVDesk\IvrsBundle\Security\Guards;
...
use Dic\UVDesk\IvrsBundle\Entity\ApiAccessCredential;
...
class IVRSAPIGuard extends AbstractGuardAuthenticator
{
    /**
    ...
    public function getCredentials(Request $request)
    {
        if (strpos(strtolower($request->headers->get('Authorization')), 'basic') === 0) {
            $authorization_key = substr($request->headers->get('Authorization'), 6);
			 $user = $this->entityManager->getRepository('UVDeskIvrsBundle:ApiAccessCredential')->getUserEmailByAccessToken($authorization_key);
            ...
            } catch (\Exception $e) { dump('Wrong credentials'); die; }
        }
